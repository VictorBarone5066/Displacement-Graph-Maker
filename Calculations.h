...
std::vector <Coords> Poscar::allPeriodicImages(std::string instructions)
{
	...
	convertToDirect();
	//to avoid having to write 26 different push backs, will add atoms to vectors and edit the vectors
...
convertToCartesian(extendC);
	...
	//removeDuplicates(periodicImages); ///shouldent need this now
...
void Poscar::fetchAtomBonds()
{
	...
	fetchBondInfoVect(bondDataInfoLoc.c_str());
	fetchAtomPairs();
...
void Poscar::fetchBondInfoVect(std::string path)
{
	if (!bondInfoVect.empty())
		return;

	std::ifstream infile((path).c_str());
	if (infile.fail())
	{
		std::cout << "fetchBondInfoVect(): could not open file with bond data for reading.  Trying to open: " << path << "\n";
